...
__device__ double sigmoid_calculation(double z) {
    return 1.0 / (1.0 + exp(-z));
}

__device__ double proba_prediction(
    double *x, 
    double *weights,
    double bias,
    int num_features
) 
{
    ...
    return sigmoid_calculation(z_gpu);
...
) 
{
    ...
    double* current_sample = &samples[sampleIdx * num_features];
    double y_pred = proba_prediction(current_sample, weights, *bias, num_features);
...
)
{
    int sampleIdx = blockIdx.x * blockDim.x + threadIdx.x;
    if(sampleIdx >= num_samples) return; //to prevent out of range
    double y_pred = proba_prediction(&samples[sampleIdx * num_features], weights, bias, num_features);
